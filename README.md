...
The cosmic web, shaped by the distribution of dark matter, defines the large-scale structure of the universe. Galaxies serve as biased tracers of this web, but their distributions are influenced by both dark matter clustering and astrophysical processes, introducing uncertainties in cosmological reconstructions. Previous work developed a diffusion generative model to reconstruct dark matter fields from stellar mass distributions, effectively marginalizing over astrophysical and cosmological uncertainties. Building on this, we condition the model on additional data types, including Fast Radio Bursts (FRBs) and gravitational lensing, to refine dark matter reconstructions. FRBs provide dispersion measures that probe ionized gas, while gravitational lensing directly traces dark matter through light deflection, with lensing shear maps derived from Astrid gas maps via a Fast Fourier Transform (FFT). These multimodal inputs improve the model’s ability to disentangle astrophysical uncertainties and enhance accuracy. Our U-Net-based diffusion model progressively denoises noisy dark matter maps using these inputs, achieving cross-correlation values exceeding 0.75 across CAMELS datasets. We evaluate the impact of varying signal-to-noise ratios (SNRs) for each modality, demonstrating improved reconstruction fidelity with higher SNRs. Finally, applying the model to Hubble Space Telescope shear maps conditioned on dark matter and Astrid lensing data validates its real-world applicability, advancing our understanding of the cosmic web and establishing a robust framework for future dark matter studies. 
