...
from datasets import SlicedDataset, PairedDataset, NPYDataset, S3PairedDataset
...
def main(args):
    ...
    region_name = 'us-east-2'  # or your bucket's region
    ...
    bucket_name = 'dark-matter-diffusion'
    # if your S3 keys are just the basenames of those local paths:
    key_mstar = 'Maps_Mstar_IllustrisTNG_LH_z=0.00.npy'
    key_mcdm = 'Maps_Mcdm_IllustrisTNG_LH_z=0.00.npy'
    s3_keys = [key_mcdm, key_mstar]

    paired_dataset = S3PairedDataset(
        s3=s3,
        bucket_name=bucket_name,
        keys=s3_keys,
        transforms=transform,
        cache_dir="/tmp/dm_diffusion_cache", 
        aws_region='us-east-2'   # or your bucket’s region
    )

    PairedDataloader = DataLoader(
        paired_dataset,
